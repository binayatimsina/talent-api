...
public class ManagerControllerIntegrationTests {
    ...
    public void afterEach() {
        ...
        ResponseEntity<List<Manager>> resp = this.managerController.getAllManagers();
        List<Manager> managers = resp.getBody();
        for (Manager manager : managers) {
            if (manager.getId() > 3l) { //there are 3 managers to start off with
