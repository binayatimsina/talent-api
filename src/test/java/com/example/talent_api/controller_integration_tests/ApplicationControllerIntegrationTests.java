...
public class ApplicationControllerIntegrationTests {
    ...
    public void afterEach() {
        ResponseEntity<List<Application>> resp = this.applicationController.getAllApplications();
        List<Application> applications = resp.getBody();
        // Clean up applications
        for (Application application : applications) {
            if (application.getId() > 5L) { // originally 5 applications
        ...
        for (User user : users) {
            if (user.getId() > 9L) { //originally 9 users
                this.userController.deleteUser(user.getId());
            }
        }
        ResponseEntity<List<Job>> resp3 = this.jobController.getAlljobs();
        List<Job> jobs = resp3.getBody();
        for (Job job : jobs) {
            if (job.getId() > 5L) { //originally 5 jobs
    ...
    void testGetApplicationById() throws Exception {
        ...
        assertThat(result.getApplication_status()).contains("Under Review");
    }
    ...
    void testGetAllApplications() throws Exception {
        ...
        assertThat(applications.length).isEqualTo(7); // Adjust based on initial state
    }
   ...
   //below
    ...
    void testAddApplication() throws Exception {
        ...
        assertThat(applications.length).isEqualTo(6); // Adjust based on initial state

        assertThat(respApplication.getStatusCode()).isEqualTo(HttpStatusCode.valueOf(201));

        assertThat(listPostAdd).isNotNull();
        assertThat(listPostAdd.length).isEqualTo(7); // Adjust based on initial state
    } 
        //above
    ...
    void testDeleteApplication() {
        ...
        assertThat(listPostDelete.length).isEqualTo(6); // Adjust based on initial state
