...
public class JobControllerIntegrationTests {
    ...
    void testGetAllJobs() throws Exception {
		...
		assertThat(jobs.length).isEqualTo(7);
    }
    ...
    void testAddJob() throws Exception {
        //remove job added in before each
        ...
        //we believe this is failing because of ResponseEntity<?>... should be ResponseEntity<Object>?

    }
    @Test
    void testGetOpenJobs() throws Exception{
        String getUrl = "http://localhost:" + port + "/jobs/open";
		...
		assertThat(openJobs.length).isEqualTo(4);
    }
    @Test
    void getOpenJobsByManager()throws Exception {
        String getUrl = "http://localhost:" + port + "/jobs/open/manager";
		...
		assertThat(openJobsByManger.length).isEqualTo(1);
    }
    @Test 
    void testSearch() throws Exception {
        String getUrl = "http://localhost:" + port + "/jobs/search";
        Job[] search = this.restTemplate.getForObject(getUrl + "?searchTerm=engineer", Job[].class);
        System.out.println("number of matching candidates: " + search.length);
        assertThat(search.length).isGreaterThan(0);
		assertThat(search.length).isEqualTo(2);
    }
    @Test
    void testUpdateUser() throws Exception {
        ...
        //update the value
        HttpHeaders hp = new HttpHeaders();
        HttpEntity<Job> reqBodyWithHeaders = new HttpEntity<>(job, hp);
    ...
    void testDeleteUser(){
        ...
        String getUrl = "http://localhost:" + port + "/jobs";
        Job[] jobsAfterDel = this.restTemplate.getForObject(getUrl, Job[].class); 
        System.out.println("number of jobs: " + jobsAfterDel.length);
		
        assertThat(jobsAfterDel.length).isGreaterThan(0);
		assertThat(jobsAfterDel.length).isEqualTo(6);
    }
